...
void _setNonblock(int fd) {
  ...
  if (ret == -1) {
    throw PtyException('fcntl3 failed.');
  }
}
...
class PtyCoreUnix implements PtyCore {
  factory PtyCoreUnix.start(
    ...
    // Child: create a new session before attaching to the pty slave.
    if (unix.setsid() == -1) {
      throw PtyException('setsid failed.');
    }
